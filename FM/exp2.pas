...
begin
  ...
  y   := 1  shl  30;
  for i := 0 to EXP2_N_SAMPLES-1 do begin
    exp2tab[(i  shl  1) + 1] := floor(y + 0.5);
    y  := y  * inc;
  end;
  for i := 0 to EXP2_N_SAMPLES - 1-1 do begin
...
begin
  y := 0;
  for i := 0 to TANH_N_SAMPLES-1 do begin
  ...
  for i := 0 to TANH_N_SAMPLES - 1-1 do begin
...
begin
  signum := x  shr  31;
  x  := x xor signum;
  if x >= (4  shl  24) then begin
    if x >= (17  shl  23) then begin
