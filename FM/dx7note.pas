...
uses env, pitchenv, fm_core, math, freqlut, exp2, controllers, fm_op_kernel,
  system.SysUtils;
...
TDx7Note = class
  ...
  pitchmodsens_,
  op                : integer;
  procedure init(const dx7patch : TBytes; midinote, velocity : integer);
  procedure compute(buf : pinteger; lfo_val, lfo_delay : integer;ctrls : TControllers);
  procedure keyup;
  procedure update_dx7(const dx7patch : TBytes; midinote, velocity : integer);
  procedure peekVoiceStatus( status : TVoiceStatus);
...
var
  ...
  raw_exp      : integer;
begin
    if (curve = 0)  or  (curve = 3) then begin
        // linear
...
procedure TDx7Note.init(const dx7patch : TBytes; midinote, velocity : integer);
var
  rates,
  levels  : TEnvArray;
  op,
...
begin
    ...
    pb := (pitchbend - $2000);
    if pb <> 0 then begin
        if ctrls.values_[kControllerPitchStep] = 0 then  begin
    ...
    // == OP RENDER ==
  for op := 0 to OP_COUNT-1 do begin
        if ctrls.opSwitch[op] = '0'  then begin
            ...
            if ampmodsens_[op] <> 0 then begin
...
procedure TDx7Note.update_dx7(const dx7patch : TBytes; midinote, velocity : integer);
var
  rates,
  levels : TEnvArray;
...
begin
    for op := 0 to OP_COUNT_DX7-1 do begin
        ...
        outlevel := dx7patch[off + 16];
        outlevel := TEnv.scaleoutlevel(outlevel);
        level_scaling := ScaleLevel(midinote, dx7patch[off + 8], dx7patch[off + 9],
                                       dx7patch[off + 10], dx7patch[off + 11], dx7patch[off + 12]);
        outlevel  := outlevel + level_scaling;
        outlevel := min(127, outlevel);
        outlevel := outlevel  shl  5;
        outlevel  := outlevel + (ScaleVelocity(velocity, dx7patch[off + 15]));
...
procedure TDx7Note.peekVoiceStatus(status: TVoiceStatus);
var
  i: integer;
begin
  for i := 0 to 5 do
